...
if (PluginLinesmanagerLine::checkPostArgumentsPermissions()) {
    ...
    if (isset($_POST['update']) or isset($_POST['add'])) {
        $error = $forward->checkArguments($_POST);
    ...
    try {
        if (isset($_POST['update'])) {
            $result = $forward->update($_POST);
        }
        if (isset($_POST['add'])) {
            $result = $forward->add($_POST);
        }
        if (isset($_POST['purge'])) {
            $result = $forward->delete($_POST);
            Html::redirect(PluginLinesmanagerForward::getFormURL());
        }
